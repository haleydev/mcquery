    ...
    public function controller(string $string)
    {
        ...
        foreach ($local as $key) {
            if ($count < $total - 1) {
                ...
                if (!file_exists(ROOT."/controllers/$folder")) {
                    mkdir(ROOT."/controllers/$folder", 0777, true);
                }
            }
            $count++;
        }
        ...
        if ($nameclass == "") {
            ...
            $confirm = true;
            if (file_exists(ROOT."/controllers/$folder$nameclass.php")) {
            ...
            if ($nameclass == 'Controller' or $nameclass == 'controller') {
                ...
                if ($confirm == true) {
                    file_put_contents(ROOT.'/controllers/' . $folder . '' . $nameclass . '.php', $file);
